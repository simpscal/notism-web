...
// Condensed mobile admin chrome: a pinned top strip (brand + Admin badge + the
// portal-wide live new-order feed pill) above the scrolling content, and a
// bottom bar of the first four icon+label nav shortcuts plus the account
// affordance. The active shortcut is a real navigation selection (aria-current)
// promoted to the ink accent. No order sidebar.
function AdminToolbarMobile({ user, onLogout, liveFeedStatus }: AdminToolbarMobileProps) {
    ...
    return (
        <>
            {/* Pinned top strip — brand + portal-wide live new-order feed pill */}
            ...
            {/* Bottom bar — flex-ordered to the foot of the shell column */}
            <div className='order-last z-30 flex h-16 shrink-0 items-center justify-around border-t border-border bg-card px-2 lg:hidden'>
                {navItems.map(item => {
