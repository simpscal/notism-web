...
import type { AdminOrderResponseModel, GetAdminOrdersForKanbanResponseModel } from '@/apis/models';
...
const paidOrder: AdminOrderResponseModel = {
    id: 'order-paid-1',
    slugId: 'ORD-PAID-001',
    userId: 'user-1',
...
const emptyColumn: GetAdminOrdersForKanbanResponseModel = { items: [], totalCount: 0 };
...
const apiUpdateResponse: AdminOrderResponseModel = {
    ...paidOrder,
    deliveryStatus: DeliveryStatusEnum.Preparing,
    paymentStatus: PaymentStatusEnum.Unpaid, // backend returns wrong paymentStatus
};

function setupKanbanHandlers(placedItems: AdminOrderResponseModel[] = [paidOrder]) {
    server.use(
...
describe('AdminOrdersKanban — drag status preservation (bug #203)', () => {
    ...
    it('preserves paymentStatus on the moved card after a successful drag-and-drop', async () => {
        ...
        const sourceData = queryClient.getQueryData<{
            pages: Array<{ items: AdminOrderResponseModel[]; totalCount: number }>;
        }>(sourceKey);
    ...
    it('does not reset paymentStatus to unpaid when API response has wrong paymentStatus', () => {
        ...
        const originalPaidOrder: AdminOrderResponseModel = {
            ...paidOrder,
            paymentStatus: PaymentStatusEnum.Paid,
        };

        const buggyApiResponse: AdminOrderResponseModel = {
            ...originalPaidOrder,
    ...
    it('falls back to the API response when the original order is not found in cache', () => {
        ...
        function buildMerged(original: AdminOrderResponseModel | undefined, updated: AdminOrderResponseModel) {
